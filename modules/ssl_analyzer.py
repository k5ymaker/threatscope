...
def grab_certificate(hostname: str, port: int = 443) -> dict:
    ...
    try:
        ...
        return _skipped_result(
            source, "cryptography library not installed. Run: pip install cryptography"
        )
    ...
    try:
        ...
        with socket.create_connection(
            (hostname, port), timeout=REQUEST_TIMEOUT
        ) as sock:
...
def ssllabs_scan(hostname: str) -> dict:
    ...
    try:
        ...
        for attempt in range(max_polls):
            ...
            if resp.status_code == 429:
                return _error_result(
                    source, "SSL Labs rate limit hit. Try again later."
                )
            ...
            if status == "ERROR":
                msg = data.get("statusMessage", "SSL Labs returned an error.")
                if "blacklist" in msg.lower():
                    msg = f"Host is blacklisted by SSL Labs. This usually happens for test domains (e.g., example.com). Try a different domain."
                return _error_result(source, msg)
            ...
            if attempt < max_polls - 1:
                console.print(
                    f"  [dim]SSL Labs status: {status} — waiting {poll_wait}s...[/dim]"
                )
                time.sleep(poll_wait)
        else:
            return _error_result(
                source, f"SSL Labs scan timed out after {max_polls} polls."
            )
        ...
        protocols = [
            ...
            for p in ep_details.get("protocols", [])
        ]
        vuln_keys = [
        ...
        details: dict = {
            ...
            "Forward Secrecy": "yes"
            if forward_secrecy >= 2
            else "partial"
            if forward_secrecy == 1
            else "no",
            "RC4 Used": "yes" if rc4_used else "no",
            "Chain Issues": str(chain_issues),
            "Vulnerabilities": ", ".join(vulns_found)
            if vulns_found
            else "none detected",
        }
...
def handle_ssl_menu() -> None:
    ...
    while True:
        ...
        if choice == "0":
            ...
            hostname = Prompt.ask(
                "[bold cyan]Enter hostname (e.g. example.com)[/bold cyan]"
            ).strip()
            ...
            if choice == "1":
                ...
                console.print(
                    "[dim]SSL Labs scan may take up to 3 minutes for a fresh scan...[/dim]"
                )
                ...
                console.print(
                    "[dim]Requesting SSL Labs analysis (may take a few minutes)...[/dim]"
                )
...
def _spinner(message: str):
    ...
    return Progress(
        SpinnerColumn(), TextColumn(f"[cyan]{message}"), transient=True, console=console
    )
